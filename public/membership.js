(function () {
  ...
  function messageDialog(title, message, promotion = false, continueLabel = '') {
    ...
    return new Promise(resolve => {
      dialog.innerHTML = `<h2 id="member-dialog-title">${esc(title)}</h2><p>${esc(message)}</p>${promotion ? `<div class="membership-dialog-promo"><p class="membership-promo-common">${copy('Gratis en todos los planes: inventario, favoritos, reporte del historial elaborado por APV y atención personalizada.','Free with every plan: inventory, favorites, history report prepared by APV and personal assistance.')}</p><div class="membership-promo-grid">${[{name:'APV Plus',price:97,fee:100,minutes:20,consultation:69,discount:30},{name:'APV Premium',price:297,fee:200,minutes:60,consultation:59,discount:40}].map(p=>`<section><h3>${p.name}</h3><strong>${usd(p.price)}${copy('/año','/year')}</strong><ul><li><b>${usd(p.fee)} ${copy('menos en tarifas APV','off APV fees')}</b> ${copy('por cada vehículo comprado','per vehicle purchased')}</li><li><b>${copy('1 asesoría gratis de','1 free consultation:')} ${p.minutes} min</b> ${copy('cada año pagado','each paid year')}</li><li>${copy('Asesorías adicionales de 60 min:','Additional 60-minute consultations:')} <b>${usd(p.consultation)}</b> ${copy('en lugar de US$99','instead of US$99')} (${usd(p.discount)} ${copy('de descuento','off')})</li></ul></section>`).join('')}</div><p class="membership-promo-note">${copy('La membresía se paga una vez al año. El descuento por vehículo aplica solo a las tarifas de APV; no a Copart, transporte ni impuestos.','Membership is billed annually. Per-vehicle discounts apply only to APV fees, not Copart, shipping or taxes.')}</p><button type="button" class="btn btn-primary" data-dialog-plans>${copy('Comparar suscripciones','Compare memberships')}</button></div>` : ''}<button type="button" class="btn btn-dark" data-dialog-continue>${esc(continueLabel || copy('Continuar gratis','Continue for free'))}</button><button type="button" class="btn btn-ghost" data-dialog-close>${copy('Cerrar','Close')}</button>`;
      let result = false;
  ...
  function render() {
    ...
    const common = [copy('Inventario de vehículos','Vehicle inventory'),copy('Favoritos en tu cuenta','Account favorites'),copy('Reporte del historial elaborado por APV Motors','Vehicle history report prepared by APV Motors'),copy('Atención personalizada','Personal assistance')];
