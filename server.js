...
const EMAIL_CODE_TTL_MS = Math.max(5, Number(process.env.EMAIL_CODE_TTL_MINUTES || 15)) * 60 * 1000;
...
const verificationSendWindows = new Map();
...
function verificationRecord(email, code) {
  const expiresAt = Date.now() + EMAIL_CODE_TTL_MS;
  const salt = crypto.randomBytes(16).toString('base64url');
  const digest = crypto.createHmac('sha256', SESSION_SECRET)
    .update(`${email}:${code}:${expiresAt}:${salt}`)
    .digest('base64url');
  return `v1.${expiresAt}.${salt}.${digest}`;
}

function verifyEmailCode(email, code, stored) {
  const parts = String(stored || '').split('.');
  if (parts.length !== 4 || parts[0] !== 'v1') {
    // Backward compatibility for pending codes generated by older deployments.
    const a = Buffer.from(String(stored || ''));
    const b = Buffer.from(String(code || ''));
    return { ok: a.length === b.length && a.length > 0 && crypto.timingSafeEqual(a, b), expired: false };
  }
  const expiresAt = Number(parts[1]);
  if (!Number.isFinite(expiresAt) || expiresAt < Date.now()) return { ok: false, expired: true };
  const expected = crypto.createHmac('sha256', SESSION_SECRET)
    .update(`${email}:${code}:${expiresAt}:${parts[2]}`)
    .digest('base64url');
  const a = Buffer.from(parts[3]);
  const b = Buffer.from(expected);
  return { ok: a.length === b.length && crypto.timingSafeEqual(a, b), expired: false };
}

function allowVerificationSend(email) {
  const now = Date.now();
  const windowStart = now - 15 * 60 * 1000;
  const recent = (verificationSendWindows.get(email) || []).filter((timestamp) => timestamp > windowStart);
  if (recent.length >= 3) return false;
  recent.push(now);
  verificationSendWindows.set(email, recent);
  return true;
}

function recordVerificationFailure(email) {
  const current = verificationFailures.get(email) || { count: 0, until: 0 };
  current.count += 1;
  if (current.count >= 5) current.until = Date.now() + 15 * 60 * 1000;
  verificationFailures.set(email, current);
}

function verificationIsBlocked(email) {
  const state = verificationFailures.get(email);
  if (!state) return false;
  if (state.until && state.until <= Date.now()) {
    verificationFailures.delete(email);
    return false;
  }
  return Boolean(state.until && state.until > Date.now());
}
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/api/config') {
      return json(res, 200, {
        ...
        emailVerificationEnabled: emailService.getSmtpConfigStatus().configured,
        debugKommo: kommoService.isDebug() || process.env.NODE_ENV !== 'production'
      });
    }

    if (req.method === 'GET' && url.pathname === '/api/email/health') {
      if (!adminAllowed(req)) return json(res, 401, { ok: false, error: 'Clave de administración inválida.' });
      try {
        await emailService.verifySmtpConnection();
        return json(res, 200, { ok: true, configured: true });
      } catch (err) {
        return json(res, 503, { ok: false, configured: emailService.getSmtpConfigStatus().configured, error: err.message });
      }
    }
    ...
    if (req.method === 'POST' && url.pathname === '/api/auth/register-request') {
      ...
      if (!allowVerificationSend(email)) return json(res, 429, { error: 'Solicitaste demasiados códigos. Intenta de nuevo en 15 minutos.' });
      ...
      const user = {
        ...
        picture: '',
        emailVerified: 0,
        verificationCode: verificationRecord(email, code),
      ...
      const response = {
        ok: true,
        message: `Hemos enviado un código de 6 dígitos a ${email}. Revisa tu bandeja de entrada.`,
        email
      };
      if (process.env.NODE_ENV !== 'production' && process.env.EMAIL_EXPOSE_DEV_CODE === 'true') response.devCode = code;
    ...
    if (req.method === 'POST' && url.pathname === '/api/auth/verify-email') {
      ...
      if (!/^\d{6}$/.test(code)) return json(res, 400, { error: 'Ingresa el código de 6 dígitos.' });
      if (verificationIsBlocked(email)) return json(res, 429, { error: 'Demasiados intentos fallidos. Solicita un código nuevo en 15 minutos.' });

      const user = catalogDb.findUserByEmail(email);
      if (!user) return json(res, 404, { error: 'No se encontró la solicitud de registro.' });
      if (user.emailVerified) return json(res, 409, { error: 'Esta cuenta ya fue verificada.' });
      const verifiedCode = verifyEmailCode(email, code, user.verificationCode);
      if (verifiedCode.expired) return json(res, 410, { error: 'El código expiró. Solicita uno nuevo.' });
      if (!verifiedCode.ok) {
    ...
    if (req.method === 'POST' && url.pathname === '/api/auth/register') {
      return json(res, 410, { error: 'El registro directo fue desactivado. Solicita primero un código de verificación.' });
    }
